...
public class AnimationController extends BaseAnimationController {
	...
	public static class AnimationDesc {
		/**
		 * Listener which will be informed when the animation is looped or
		 * ended.
		 */
		...
		/**
		 * The speed at which to play the animation (can be negative), 1.0 for
		 * normal speed.
		 */
		public float speed;
		...
		public float time;
		/**
		 * The number of remaining loops, negative for continuous, zero if
		 * stopped.
		 */
		...
		public float update(float delta) {
			if (loopCount != 0 && animation != null) {
				...
				for (int i = 0; i < loops; i++) {
					...
					if (loopCount == 0) {
						final float result = ((loops - 1) - i) * duration
								+ (diff < 0f ? duration - time : time);
	...
	private AnimationDesc obtain(final Animation anim, int loopCount,
	...
	private AnimationDesc obtain(final String id, int loopCount, float speed,
	...
	/**
	 * @param delta
	 *            The time elapsed since last update, change this to alter the
	...
	public void update(float delta) {
		...
		if (previous != null
				&& ((transitionCurrentTime += delta) >= transitionTargetTime)) {
			animationPool.free(previous);
			previous = null;
		}
		if (previous != null)
			applyAnimations(previous.animation, previous.time,
					current.animation, current.time, transitionCurrentTime
	...
	public void update(float delta, float time) {
		if (current == null || current.loopCount == 0
			...
			return;
		updating = true;
		applyAnimation(current.animation, time);
		updating = false;
	}

	/** Set the active animation, replacing any current animation. */
	public void setAnimation(final String id, int loopCount, float speed,
			final AnimationListener listener) {
		setAnimation(obtain(id, loopCount, speed, listener));
	}

	/** Set the active animation, replacing any current animation. */
	protected void setAnimation(final Animation anim, int loopCount,
			float speed, final AnimationListener listener) {
	...
	protected void setAnimation(final AnimationDesc anim) {
		if (updating)
			throw new GdxRuntimeException(
					"Cannot change animation during update");
		if (current == null)
			current = anim;
		else {
	...
	/**
	 * Changes the current animation by blending the new on top of the old
	 * during the transition time.
	 */
	public void animate(final String id, int loopCount, float speed,
	 ...
	 * during the transition time.
	 */
	protected void animate(final Animation anim, int loopCount, float speed,
			final AnimationListener listener, float transitionTime) {
	...
	/**
	 * Queue an animation to be applied when the current is finished. If current
	 * is continuous it will be synced on next loop.
	 */
	public void queue(final String id, int loopCount, float speed,
			final AnimationListener listener, float transitionTime) {
	 ...
	 * is continuous it will be synced on next loop.
	 */
	protected void queue(final Animation anim, int loopCount, float speed,
			final AnimationListener listener, float transitionTime) {
	...
	public void action(final String id, int loopCount, float speed,
			final AnimationListener listener, float transitionTime) {
		action(obtain(id, loopCount, speed, listener), transitionTime);
	}

	/** Apply an action animation on top of the current animation. */
	protected void action(final Animation anim, int loopCount, float speed,
			final AnimationListener listener, float transitionTime) {
